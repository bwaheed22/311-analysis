...
#' Retrieve weather data from the NOAA API
#' 
#' Returns daily temperature, precipitation, or wind data using the NOAA API. Must set API token via Sys.setenv(token_noaa = '<token>') prior to use.
#'
#' @param date_start the starting date to retrieve data for
#' @param date_end the end date to retrieve data for
#' @param weather_type one of c('temperature', 'precipitation', 'wind')
#'
#' @return a data frame
#' @export
#' 
...
#' get_NOAA('2021-09-10', '2021-09-15', 'temperature')
get_NOAA <- function(date_start, date_end, weather_type = c('temperature', 'precipitation', 'wind')){
  # must set API token as environment variable via Sys.setenv(token_noaa = '<token>')
  # get token here https://www.ncdc.noaa.gov/cdo-web/token

  if (!inherits(as.Date(date_start), 'Date') | !inherits(as.Date(date_end), 'Date')) stop('date_start & date_end must be coercible to dates')
  
  # TODO: if more than 1000 days then split into multiple calls?
  
  # convert weather type plain text to NOAA variable
  # TODO: wind doesn't work yet; probably a different dataset
  weather_type <- switch(weather_type,
                         'temperature' = 'TMAX',
                         'precipitation' = 'PRCP',
                         'wind' = 'WSF1')
  ...
  datatype <- paste0('datatypeid=', weather_type)
  units <- 'standard'
  limit <- 'limit=1000' # 1000 is the max
  url_base <- 'https://www.ncdc.noaa.gov/cdo-web/api/v2/data?'
  url_complete <- paste0(url_base, dataset, "&", datatype, '&', station, "&", date_start, "&", date_end, '&', units, '&', limit)
  ...
  # TODO: the returned values are strange -- wrong units?
...
weather_temperature <- function(.date){
  ...
  # get_NOAA(, weather_type = 'temperature')

}
weather_precipitation <- function(.date){
  is_historical <- .date <= Sys.Date()
  
  # get_NOAA(, weather_type = 'precipitation')
  
}
weather_wind <- function(.date){
  is_historical <- .date <= Sys.Date()
  
  # get_NOAA(, weather_type = 'wind')
