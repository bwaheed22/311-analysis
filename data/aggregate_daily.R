...
# select top agencies with > 25,000 calls (~17 calls per day)
...
calls_daily <- calls_daily %>% 
  ...
  group_by(date, agency, complaint_type) %>% summarise(n = sum(n)) %>% ungroup()


calls_daily %>% filter(agency == 'DEP') %>% 
  ggplot(., aes(x = date, y = n)) +
  facet_wrap(agency~complaint_type, scales = "free") + 
  geom_line() + 
  theme_bw() +
  labs(title = "Calls by Agency per day",
